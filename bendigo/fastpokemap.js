...
L.HtmlIcon = L.Icon.extend({
    ...
    createIcon: function () {
        ...
        if (this.options.hide) {
            div.innerHTML = '<div class="displaypokemon hidden" data-pokeid="' + this.options.pokemonid + '">' + '<div class="pokeimg">' + '<img src="data:image/png;base64,' + pokemonPNG[this.options.pokemonid] + '" />' + '</div>' + '<div class="remainingtext" data-expire="' + this.options.expire + '"></div>' + '</div>';
        } else {
            div.innerHTML = '<div class="displaypokemon" data-pokeid="' + this.options.pokemonid + '">' + '<div class="pokeimg">' + '<img src="data:image/png;base64,' + pokemonPNG[this.options.pokemonid] + '" />' + '</div>' + '<div class="remainingtext" data-expire="' + this.options.expire + '"></div>' + '</div>';
        }
        return div;
    },
    ...
    $.getJSON("https://cache.fastpokemap.se/?key=" + window.fingerprint + "&ts=" + window.salt + "&compute=" + window.myIp + "&lat=" + cp.lat + "&lng=" + cp.lng, function (data) {
        if (data.length >= 1) {
            ...
            for (i in data) {
                ...
                if (new Date(cachedSpawn.expireAt).getTime() > new Date().getTime()) {
...
function getPokemon(lat, lng) {
    $.ajax({
        ...
        setTimeout(function () {
            if ($('.scan').data("failid") == currfailure) {
...
function onLocationFound(event) {
    ...
    if (!isScanning && !firstTime) {
        if (!$('.scan').hasClass('active')) {
            $('.scan').removeClass('success').removeClass('failed').addClass('active');
        }
...
function updateTime() {
    ...
    for (j in shownMarker) {
        ...
        if (expire <= now) {
            ...
            if (visibleArea.contains(shownMarker[j].marker.getLatLng())) {
                if (!$(shownMarker[j].marker._icon).children().hasClass("hidden")) {
                    element = $(shownMarker[j].marker._icon).find(".remainingtext");
                    ts = (element.data("expire") / 1000 | 0) - (now / 1000 | 0);
                    minutes = component(ts, 60) % 60,
                    seconds = component(ts, 1) % 60;
                    if (seconds < 10)
                        seconds = '0' + seconds;
                    element.html(minutes + ":" + seconds);
    ...
    while (toDelete.length) {
        shownMarker.splice(toDelete.pop(), 1);
    }
}
...
$(function () {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent)) {
        $('.slicknav_menu').prepend('<div class="message">Get alerts when near Pokemon<br /> and Pokestops! Install <strong><a href="https://goo.gl/CzM9bc">GoChat</a></strong>!</div>');
    } else if (/(android)/i.test(navigator.userAgent)) {
        $('.slicknav_menu').prepend('<div class="message">Get alerts when near Pokemon<br /> and Pokestops! Install <strong><a href="https://goo.gl/HhBwtt">GoChat</a></strong>!</div>');
    }
    ...
    $.get("https://gist.githubusercontent.com/anonymous/50c284e815df6c81aa53497a305a29f2/raw", function (data) {
    ...
    $('#applyfilter').on('click', function () {
        ...
        Cookies.set('filter', filterlist.join(cookiedelimchar), {
            expires: 3650,
            path: ''
        });
        filterdict = {};
