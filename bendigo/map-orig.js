...
function initMap () { // eslint-disable-line no-unused-vars
  ...
  if (!searchMarker.rangeCircle) {
    searchMarker.rangeCircle = addRangeCircle(searchMarker, map, 'nearby')
  }

  if (!searchMarker.visibleRangeCircle) {
    searchMarker.visibleRangeCircle = addRangeCircle(searchMarker, map, 'visible')
  }
...
function createSearchMarker () {
  ...
  google.maps.event.addListener(searchMarker, 'dragend', function () {
    ...
    if (searchMarker.visibleRangeCircle) {
      searchMarker.visibleRangeCircle.setCenter(newLocation)
    }
    changeSearchLocation(newLocation.lat(), newLocation.lng())
      ...
      .fail(function () {
        if (oldLocation) {
          ...
          if (searchMarker.rangeCircle) {
            searchMarker.rangeCircle.setCenter(oldLocation)
          }
          if (searchMarker.visibleRangeCircle) {
            searchMarker.visibleRangeCircle.setCenter(oldLocation)
          }
        }
      })
  })
...
function redrawSearchMarker () {
  if (isRangeActive(map)) {
    if (searchMarker.rangeCircle) {
      searchMarker.rangeCircle.setMap(map)
    }
    if (searchMarker.visibleRangeCircle) {
      searchMarker.visibleRangeCircle.setMap(map)
    }
  } else {
    if (searchMarker.rangeCircle) {
      searchMarker.rangeCircle.setMap(null)
    }
    if (searchMarker.visibleRangeCircle) {
      searchMarker.visibleRangeCircle.setMap(null)
    }
  }
}
...
function changeLocation (lat, lng) {
  ...
  changeSearchLocation(lat, lng).done(function () {
    ...
    if (searchMarker.rangeCircle) {
      searchMarker.rangeCircle.setCenter(loc)
    }
  })
}
